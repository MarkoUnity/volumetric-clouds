Shader "Custom/CloudShader"
{
	Properties
	{
		...
		_CloudMinSamples("Min Sample Count", Range(8, 32)) = 16
		_CloudMaxSamples("Max Sample Count", Range(32, 128)) = 64
		_CloudDistanceScale("Sample Distance Scale", Range(0.1, 2.0)) = 1.0
		_CloudDensityScale("Density Sample Scale", Range(0.1, 2.0)) = 1.0
	}
		...
		SubShader
		{
			...
			float _CloudMinSamples;
			float _CloudMaxSamples;
			...
			float4 frag (Interpolator i) : SV_Target
			{
				...
				// Calculate adaptive sample count based on viewing angle and distance
				float viewAngleFactor = 1.0 - abs(viewDir.y); // 0 when looking straight up/down, 1 when looking horizontally
				float distanceFactor = saturate(distance(_WorldSpaceCameraPos, worldPos.xyz) / _CloudDistanceScale);
				
				// Combine factors to determine sample count
				float sampleCountFactor = lerp(viewAngleFactor, distanceFactor, _CloudDensityScale);
				int sample_count = lerp(_CloudMinSamples, _CloudMaxSamples, sampleCountFactor);
				sample_count = max(8, min(128, sample_count)); // Clamp to valid range
